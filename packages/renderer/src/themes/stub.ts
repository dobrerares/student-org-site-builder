...
[data-block="contactCard"] .contact-card__layout {
  display: grid;
  gap: var(--space-lg);
}
...
[data-block="contactCard"] .contact-card__channel {
  display: flex;
  align-items: center;
  gap: var(--space-sm);
}
[data-block="contactCard"] .contact-card__icon {
  display: inline-flex;
  align-items: center;
...
[data-block="contactCard"] .contact-card__channel-body {
  min-width: 0;
...
[data-block="richText"] .rich-text :is(ul, ol) {
  padding-left: var(--space-lg);
}
[data-block="richText"] .rich-text li > p {
  margin: 0 0 var(--space-xs) 0;
}
[data-block="richText"] .rich-text li > p:last-child {
  margin-bottom: 0;
}
[data-block="richText"] .rich-text s {
  text-decoration-color: var(--color-muted);
}
[data-block="richText"] .rich-text-figure {
  margin: 0 0 var(--space-md) 0;
}
[data-block="richText"] .rich-text-figure img {
  display: block;
  width: 100%;
  height: auto;
  border-radius: var(--radius-sm);
}
[data-block="richText"] .rich-text-figure figcaption {
  margin-top: var(--space-xs);
  color: var(--color-muted);
  font-size: 0.9em;
}
/*
 * Unsupported content is an empty element: the Renderer will not invent a
 * rendering for content it cannot read, and ADR 0048 forbids simplifying it
 * away. It also never reaches a built site — it is a non-overridable export
 * blocker — so these rules only ever apply in the editor preview, where the
 * author needs to see that something is there.
 */
[data-block="richText"] .rich-text-unsupported {
...
[data-block="articleList"] .article-list__title {
  font-family: var(--font-headline);
  margin: 0 0 var(--space-sm) 0;
  color: var(--color-primary);
}
[data-block="articleList"] .article-list__intro {
  margin: 0 0 var(--space-md) 0;
  color: var(--color-fg);
}
[data-block="articleList"] .article-list__empty {
  margin: 0;
  color: var(--color-muted);
}
...
[data-block="articleList"] .article-list__item {
  display: flex;
}
.article-card {
  display: flex;
