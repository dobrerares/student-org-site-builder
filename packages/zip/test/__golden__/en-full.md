...
Your site was built for `https://historipol.ro`. Make sure the final public address (either `*.pages.dev` or your custom domain) matches that value. If you pick a different address, re-export from the editor with the new `siteUrl` so the canonical links and sitemap are correct.
...
In the Pages project, open the **Custom domains** tab → **Set up a custom domain** and enter `historipol.ro`. Cloudflare shows a CNAME record to add at your DNS provider — typically `<your-domain>` with the value `<project-name>.pages.dev`. Add it in your registrar's panel (GoDaddy, Namecheap, RoTLD, etc.). DNS propagation can take from a few minutes to a few hours.
