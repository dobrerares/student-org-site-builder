...
export type FieldNode =
  | { kind: "string"; name: string; path: string[]; optional: boolean }
  | { kind: "number"; name: string; path: string[]; optional: boolean }
  | { kind: "boolean"; name: string; path: string[]; optional: boolean }
  | {
      kind: "enum";
      name: string;
      path: string[];
  ...
  | { kind: "object"; name: string; path: string[]; optional: boolean; fields: FieldNode[] }
  | { kind: "array"; name: string; path: string[]; optional: boolean; element: FieldNode };
...
export function fieldsFromSchema(schema: ZodType): FieldNode[] {
  ...
  return Object.entries(introspect.shape).map(([name, child]) =>
    nodeFor(name, child, [name]),
  );
}
...
function nodeFor(name: string, schema: ZodType, path: string[]): FieldNode {
  ...
  let optional = false;
  // Strip optional/nullable wrappers and track the optional flag.
