 ...
 *
 * The Custom Block leg (a Partners Block with editable fields, issue #106)
 * is NOT covered here: the declaration contract had not merged when this
 * spec was written. See the acceptance document's "not yet covered" section.
 */
...
test("a developer's Theme package goes through import, editing, both previews, export and re-import", async ({
  ...
  await live.locator('[data-site-nav] a[href="/despre/"]').click();
  ...
  const data = JSON.parse(new TextDecoder().decode(await archive.read("data.json"))) as {
    theme: { id: string };
    pages: { blocks: { type: string; variant?: string; data: { title?: string } }[] }[];
  };
  ...
  await expect(reopened.getByTestId("block-row")).toHaveCount(2);
